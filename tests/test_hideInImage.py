...
from HystrixBox.Tools.hide_in_image import find_files
...
def test_find_files_hiddenFileJpg(capfd, tmpdir):
    ...
    find_files('../examples/HideInImage/hiddenInImage.jpg', file.strpath)
    out, err = capfd.readouterr()
    assert (
        filecmp.cmp(file.strpath + '.jpg', '../examples/HideInImage/hiddenFile.jpg')), 'Extracted files are not equals'
    assert (out == TEST1), 'stdout are not equals'


def test_find_files_noHidden(capfd, tmpdir):
    file = tmpdir.join('output')
    find_files('../examples/strings.jpg', file.strpath)
    out, err = capfd.readouterr()
...
def test_find_files_noImage_txt(capfd, tmpdir):
    file = tmpdir.join('output')
    find_files('../examples/extractor.txt', file.strpath)
    out, err = capfd.readouterr()
...
def test_find_files_corruptedImageTrailer(capfd, tmpdir):
    file = tmpdir.join('output')
    find_files('../examples/HideInImage/corruptedImageTrailer.jpg', file.strpath)
    out, err = capfd.readouterr()
...
def test_find_files_noImage_zip(capfd, tmpdir):
    file = tmpdir.join('output')
    find_files('../examples/recursivezip.zip', file.strpath)
    out, err = capfd.readouterr()
...
def test_find_files_hiddenFilePng(capfd, tmpdir):
    file = tmpdir.join('output')
    find_files('../examples/HideInImage/hiddenInImage.png', file.strpath)
    out, err = capfd.readouterr()
    assert (
        filecmp.cmp(file.strpath + '.jpg', '../examples/HideInImage/hiddenFile.jpg')), 'Extracted files are not equals'
    assert (out == TEST1), 'stdout are not equals'
...
def test_find_files_hiddenFileCorrupted(capfd, tmpdir):
    file = tmpdir.join('output')
    find_files('../examples/HideInImage/hiddenInImageCorrupted.png', file.strpath)
    out, err = capfd.readouterr()
